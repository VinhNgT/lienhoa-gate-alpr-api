...
# python run-image.py --image $SDK_PATH/assets/images/lic_us_1280x720.jpg --assets $SDK_PATH/assets --openvino_enabled True

import site
import os

site.addsitedir(os.path.join(os.getenv("SDK_PATH"), "python"))
site.addsitedir(os.getenv("LD_LIBRARY_PATH"))


import ultimateAlprSdk  # type: ignore
import argparse
import json
import os.path
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--image", required=True, help="Path to the image with ALPR data to recognize"
    )
    parser.add_argument(
        "--assets",
        required=False,
    ...
    parser.add_argument(
        "--tokenfile", required=False, default="", help="Path to license token file"
    )
    parser.add_argument(
        "--tokendata", required=False, default="", help="Base64 license token data"
    )
